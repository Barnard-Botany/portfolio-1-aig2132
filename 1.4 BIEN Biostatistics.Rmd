...
```{r}
Artocarpus_altilis_full <- BIEN_occurrence_species(species = "Artocarpus altilis",cultivated = T,only.new.world = T ,all.taxonomy = T,native.status = T,observation.type = T,political.boundaries = T)
...
1. Colombia 
```{r}
Colombia<-BIEN_list_country(country = "Colombia")
head(Colombia)
tail(Colombia)
```
2. Oaxaca (the state in Mexico that my mom grew up, and place of where my favorite movie Coco takes place)
```{r}
Oaxaca <- BIEN_occurrence_state(country="Mexico", state="Oaxaca")
head(Oaxaca)
tail(Oaxaca) 
Oaxaca 
```
3. Will do for Manhattan 
```{r}
Manhattan <-BIEN_list_county(country = "United States", state="New York", county="Manhattan", native.status=TRUE)
head(Manhattan)
tail(Manhattan) 
```
